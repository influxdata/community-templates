...
def parse_data(data):
    ...
    try:
        if data['global_stats']['solo']['placetop1'] >= 0:
    ...
    try:
        if data['global_stats']['duo']['placetop1'] >= 0:
    ...
    try:
        if data['global_stats']['squad']['placetop1'] >= 0:
...
with io.open('/etc/telegraf/players.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        if data['account']['level'] and data['global_stats']:
